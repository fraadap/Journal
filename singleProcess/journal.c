...
int main(int argc, char *argv[])
{
    ...
    int m = 0;         // byte letti dal file
    do
    {
        m = read(file, &c, 1); // lettura bloccante dalla pipe

        if (m <= 0)
        { // Se non ha letto dal file nessun byte assegno \0 al carattere che vuol dire che il file è finito
            c = '\0';
        }
