...
struct ColumnNode *addRow(char *bufferRow, int rpc, struct ColumnNode *firstColumn, int cpp, int end, int dcc, char *outputFile)
{
    ...
    {
        wait(NULL); //il processo padre aspetta che il processo figlio finisca di stampare la pagina precedente
        int pid = fork();
        if (pid == -1)
        {
